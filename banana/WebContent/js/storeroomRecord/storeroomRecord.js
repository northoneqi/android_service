...
Ext.QuickTips.init();
storeroomRecord=Ext.extend(storeroomRecordUi,{
	initComponent: function() {
		storeroomRecord.superclass.initComponent.call(this);
				 ...
				 saveButtion.on("click",function(){
					...
					if(formPanel.isValid()){
						formPanel.submit({
							...
							waitTitle : '系统提示',// 标题
							method:'POST',
							url:ctx+"/supplyChain/storeroomRecord/save.do",
		...
		Ext.getCmp("edit_button").on("click",function(){
			 ...
			 if(rowid!="" && c.length == 1){
				 win.show();
				 Ext.getCmp("storeroomRecordId").setValue(rowid);
		...
		Ext.getCmp("delete_button").on("click",function(){
			 ...
			 if(rowid!=""){
				 Ext.Msg.confirm('系统提示', '您确定要删除吗?', function (opt) {
	                 if (opt == 'yes') {
	                    Ext.Ajax.request({
							 url:ctx+"/supplyChain/storeroomRecord/delete.do",
							 params:{ids:rowid},
...
function loadForm(id) {
	...
	formPanel.load( {
		...
		waitTitle : '提示',//标题
		url:ctx+"/supplyChain/storeroomRecord/load.do",
...
function getCheckRowId(){
	 ...
	 if(sm.getSelected()){
		 ...
		 for(var i = 0 ; i < recs.length ; i++){
			 rowId += recs[i].get("storeroomRecordId");
