...
keel.UploadPanel = function(cfg){
	...
	this.gp = new Ext.grid.EditorGridPanel({
		...
		store: new Ext.data.Store({
			fields:['id','name','type','size','state','percent', 'totalPage']
		}),
	    columns: [
	        ...
	        {header: '总页数', width: 60, sortable: true,dataIndex: 'totalPage', menuDisabled:true, 
	        ...
	        {header: '&nbsp;',width:40,dataIndex:'id', menuDisabled:true,/*renderer:this.formatDelBtn,*/
	        	...
	        	items: [{
	        		text: '移除',
	        		icon: 'images/ext/close.png',
	        		handler: function(grid, rowIndex, colIndex){
			        	Ext.MessageBox.confirm("移除", "您确定要移除此文件吗？",function(btn){
							if(btn == "yes"){
								var rec = grid.getStore().getAt(rowIndex);
								console.log(rec);
								grid.getStore().removeAt(rowIndex);
								me.swfupload.cancelUpload(rec.get("id"), false);
								if(rec.get("id").length == 32){
									Ext.ux.Ajax.request("sys/file/delete.do", {id: rec.get("id")}, function(action, form){
	...
	keel.UploadPanel.superclass.constructor.call(this,{				
		tbar : [
			{text:'添加文件',iconCls:'fileIcon',ref:'../addBtn'},'-',
			{text:'上传',ref:'../uploadBtn',iconCls:'uploadIcon',handler:this.startUpload,scope:this},'-',
			{text:'停止上传',ref:'../stopBtn',iconCls:'stopUploadIcon',handler:this.stopUpload,scope:this,disabled:true},'-',
			{text:'删除所有',ref:'../deleteBtn',iconCls:'deleteIcon',handler:this.deleteAll,scope:this},'-'
		],
		layout : 'border',
		items : [this.code, this.gp],
		listeners : {
...
Ext.extend(keel.UploadPanel,Ext.Panel,{
 	...
 	onUploadStart : function(file) {
	   ...
	   for(var i=0;i<ds.getCount();i++){
			...
			if(rec.get('id') == file.id){
				this.addPostParam("name", encodeURIComponent(rec.get('name')));  //添加上传参数
				this.addPostParam("type", rec.get('type'));  //添加上传参数
				this.addPostParam("size", rec.get('size'));  //添加上传参数
				this.addPostParam("totalPage", rec.get('totalPage'));  //添加上传参数
				this.addPostParam("code", me.code.getValue());  //添加上传参数
			}
	   }
	...
	onUploadSuccess : function(file, serverData) {
		...
		if (result.success) {	
			me.code.setValue(result.data.code);
			for(var i=0;i<ds.getCount();i++){
				var rec =ds.getAt(i);
				if(rec.get('id')==file.id){
