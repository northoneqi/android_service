...
Ext.QuickTips.init();
singleTableTest=Ext.extend(singleTableTestUi,{
	initComponent: function() {
		singleTableTest.superclass.initComponent.call(this);
				 ...
				 saveButtion.on("click",function(){
					...
					if(formPanel.isValid()){
						formPanel.submit({
							...
							waitTitle : '系统提示',// 标题
							method:'POST',
							url:ctx+"/auto/singleTableTest/save.do",
		...
		Ext.getCmp("edit_button").on("click",function(){
			 ...
			 if(rowid!="" && c.length == 1){
				 win.show();
				 Ext.getCmp("singleTableTestId").setValue(rowid);
		...
		Ext.getCmp("delete_button").on("click",function(){
			 ...
			 if(rowid!=""){
				 Ext.Msg.confirm('系统提示', '您确定要删除吗?', function (opt) {
	                 if (opt == 'yes') {
	                    Ext.Ajax.request({
							 url:ctx+"/auto/singleTableTest/delete.do",
							 params:{ids:rowid},
...
function loadForm(id) {
	...
	formPanel.load( {
		...
		waitTitle : '提示',//标题
		url:ctx+"/auto/singleTableTest/load.do",
...
function getCheckRowId(){
	 ...
	 if(sm.getSelected()){
		 ...
		 for(var i = 0 ; i < recs.length ; i++){
			 rowId += recs[i].get("singleTableTestId");
