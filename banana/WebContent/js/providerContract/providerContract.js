...
Ext.QuickTips.init();
providerContract=Ext.extend(providerContractUi,{
	initComponent: function() {
		providerContract.superclass.initComponent.call(this);
				 ...
				 saveButtion.on("click",function(){
					...
					if(formPanel.isValid()){
						formPanel.submit({
							...
							waitTitle : '系统提示',// 标题
							method:'POST',
							url:ctx+"/supplyChain/providerContract/save.do",
		...
		Ext.getCmp("exportexcelButton").on("click",function(){
			window.location.href=ctx+'/supplyChain/providerContract/exportCompExcel.do';
			Ext.Ajax.request({
		...
		Ext.getCmp("edit_button").on("click",function(){
			 ...
			 if(rowid!="" && c.length == 1){
				 ...
				 grid2.getStore().load();
				 win.show();
				 Ext.getCmp("providerContractId").setValue(rowid);
		...
		Ext.getCmp("delete_button").on("click",function(){
			 ...
			 if(rowid!=""){
				 Ext.Msg.confirm('系统提示', '您确定要删除吗?', function (opt) {
	                 if (opt == 'yes') {
	                    Ext.Ajax.request({
							 url:ctx+"/supplyChain/providerContract/delete.do",
							 params:{ids:rowid},
...
function loadForm(id) {
	...
	formPanel.load( {
		...
		waitTitle : '提示',//标题
		url:ctx+"/supplyChain/providerContract/load.do",
...
function loadFormCh(id) {
	...
	formPanel.load( {
		waitMsg : '正在加载数据请稍后',//提示信息
		waitTitle : '提示',//标题
		url:ctx+"/supplyChain/contractProductDetail/load.do",
		params:formPanel.getFieldValues(),
		method : 'POST',//请求方式
		failure : function(form, action) {//加载失败的处理函数
			Ext.Msg.alert('系统提示', '数据加载失败');
...
function getCheckRowId(){
	 ...
	 if(sm.getSelected()){
		 ...
		 for(var i = 0 ; i < recs.length ; i++){
			 rowId += recs[i].get("providerContractId");
...
function getCheckRowId2(){
	 ...
	 if(sm.getSelected()){
		 var recs=sm.getSelections(); // 把所有选中项放入数组 
		 for(var i = 0 ; i < recs.length ; i++){
			 rowId += recs[i].get("contractProductDetailId");
			 if(i+1<recs.length)rowId +=",";
		 }
	 }
	 return rowId;
