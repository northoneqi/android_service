					...
					items: [{
						...
						items:[
					        {name: 'id', hidden: true, allowBlank:true},
   					        ...
   					        {fieldLabel:'<span style="color:red">*</span>收款人', name: 'writerPeopleCode', value: userCode, hidden: true},
   					        {fieldLabel:'<span style="color:red">*</span>收款人', name: 'writePeople', value: userName},
   					        createComboTree({
   					        	id: 'categoryCombo',
						...
						xtype : 'container',
						items:[
						    {fieldLabel:'<span style="color:red">*</span>附件张数', name: 'attachNum', value: 0},
						    {fieldLabel:'<span style="color:red">*</span>收款人电话', name: 'writePeopleTel'},
       		...
       		buttons:[{
				...
				handler: function(){
					...
					for(var i = 0; i < datas.items.length; i++) {
						...
						details += "{gatheringStage: '"+data.gatheringStage+"'," +
								"startDate: '"+new Date(data.startDate).format('Y-m-d')+"'," +
								"receivablesMoney: "+data.receivablesMoney+"," +
								"digest: '"+data.digest+"'," +
								"remark: '"+data.remark+"'}";
					}
					
					details += "]";
					Ext.ux.Form.submitForm(Ext.getCmp("billFormPanel"), "budget/receiptDocument/save.do", 
				  ...
				  handler: function(){
					  ...
					  for(var i = 0; i < datas.items.length; i++) {
							...
							details += "{id: '" +id+"',"+
									"gatheringStage: '"+data.gatheringStage+"'," +
									"startDate: '"+new Date(data.startDate).format('Y-m-d')+"'," +
									"receivablesMoney: "+data.receivablesMoney+"," +
									"digest: '"+data.digest+"'," +
									"remark: '"+data.remark+"'}";
						...
						Ext.ux.Form.submitForm(Ext.getCmp("billFormPanel"), "budget/receiptDocument/update.do", 
		            		function(form, action){
		            			Ext.Msg.alert('消息', action.result.data.instruction);
		            			Ext.getCmp("addWindow").close();
		            			Ext.getCmp("mainGrid").getStore().load();
