			...
			items:[{
				...
				items:[{
					...
					items: [{
						...
						items:[
							...
							{fieldLabel:'<span style="color:red">*</span>预算起始日期', name: 'startBudgetDate', xtype: 'datefield', value: new Date(), format: 'Y-m-d'}
						 ]
					}, {
						defaults : {allowBlank : false,xtype : 'textfield', msgTarget : 'side',labelWidth:80, anchor : anchor_w},
						labelAlign : 'right',
						...
						items:[
							...
							Ext.ux.form.ComboBox({
				 		    ...
				 		    {fieldLabel:'<span style="color:red">*</span>预算截止日期', name: 'endBudgetDate', xtype: 'datefield', value: new Date(), format: 'Y-m-d'}
						 ]
					}, {
						defaults : {allowBlank: false,xtype : 'textfield', msgTarget : 'side',labelWidth:80, anchor : anchor_w},
						labelAlign : 'right',
						...
						items:[
						    ...
						    {fieldLabel:'<span style="color:red">*</span>期间', name: 'period'}
						 ]
					}, {
						labelAlign : 'right',
						layout:'form',
       		...
       		buttons:[{
				...
				handler: function(){
					...
					details += "]"
					Ext.ux.Form.submitForm(Ext.getCmp("billFormPanel"), "budget/monthExpenseDocument/save.do", 
				  ...
				  handler: function(){
					  ...
					  for(var i = 0; i < datas.items.length; i++) {
						...
						Ext.ux.Form.submitForm(Ext.getCmp("billFormPanel"), "budget/monthExpenseDocument/update.do", 
		            		function(form, action){
		            			Ext.Msg.alert('消息', action.result.data.instruction);
		            			Ext.getCmp("addWindow").close();
		            			Ext.getCmp("mainGrid").getStore().load();
