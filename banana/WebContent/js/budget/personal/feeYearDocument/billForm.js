		...
		Ext.applyIf(this,{
					...
					items: [{
						...
						items:[
					        {name: 'id', hidden: true, allowBlank: true},
   					        ...
   					        createComboTree({
								...
								fieldLabel: '<span style="color:red">*</span>大区',
						...
						items:[
						    ...
						    createComboTree({
								...
								fieldLabel: '<span style="color:red">*</span>编撰部门d',
			...
			buttons:[{
				...
				handler: function(){
					...
					details += "]"
					Ext.ux.Form.submitForm(Ext.getCmp("billFormPanel"), "budget/feeYearDocument/save.do", 
				  ...
				  handler: function(){
					  ...
					  for(var i = 0; i < datas.items.length; i++) {
						...
						Ext.ux.Form.submitForm(Ext.getCmp("billFormPanel"), "budget/feeYearDocument/update.do", 
		            		function(form, action){
		            			Ext.Msg.alert('消息', action.result.data.instruction);
		            			Ext.getCmp("addWindow").close();
		            			Ext.getCmp("mainGrid").getStore().load();
