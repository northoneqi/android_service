			...
			items:[{
					...
					items: [{
						...
						items:[
						    ...
						    createComboTree({
								...
								fieldLabel: '<span style="color:red">*</span>所属办事处部门',
       		...
       		buttons:[{
				...
				handler: function(){
					...
					details += "]"
					Ext.ux.Form.submitForm(Ext.getCmp("billFormPanel"), "budget/reimburseDocument/save.do", 
				  ...
				  handler: function(){
					  ...
					  for(var i = 0; i < datas.items.length; i++) {
						...
						Ext.ux.Form.submitForm(Ext.getCmp("billFormPanel"), "budget/reimburseDocument/update.do", 
		            		function(form, action){
		            			Ext.Msg.alert('消息', action.result.data.instruction);
		            			Ext.getCmp("addWindow").close();
		            			Ext.getCmp("mainGrid").getStore().load();
