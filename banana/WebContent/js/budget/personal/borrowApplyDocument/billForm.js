			...
			items:[{
       		...
       		buttons:[{
				...
				handler: function(){
					...
					details += "]"
					Ext.ux.Form.submitForm(Ext.getCmp("billFormPanel"), "budget/borrowApplyDocument/save.do", 
				  ...
				  handler: function(){
					  ...
					  for(var i = 0; i < datas.items.length; i++) {
						...
						Ext.ux.Form.submitForm(Ext.getCmp("billFormPanel"), "budget/borrowApplyDocument/update.do", 
		            		function(form, action){
		            			Ext.Msg.alert('消息', action.result.data.instruction);
		            			Ext.getCmp("addWindow").close();
		            			Ext.getCmp("mainGrid").getStore().load();
