		...
		Ext.applyIf(this,{
			...
			items:[{
				...
				items: [{
					...
					items:[
				        ...
				        //{name: 'auditState', hidden: true, value: 0, allowBlank: true},
					...
					columnWidth : 1,
					xtype : 'container',
					defaults : {allowBlank:true,type : 'textfield', msgTarget : 'side',labelWidth:80, anchor : anchor_w},
					items: [
			...
			buttons:[{
				...
				handler: function(){
					...
					Ext.ux.Form.submitForm(Ext.getCmp("billFormPanel"), "budget/projectBudgetDocument/save.do", 
				  ...
				  handler: function(){
					  ...
					  for(var i = 0; i < datas.items.length; i++) {
						...
						Ext.ux.Form.submitForm(Ext.getCmp("billFormPanel"), "budget/projectBudgetDocument/update.do", 
		            		function(form, action){
		            			Ext.Msg.alert('消息', action.result.data.instruction);
		            			Ext.getCmp("addWindow").close();
		            			Ext.getCmp("mainGrid").getStore().load();
