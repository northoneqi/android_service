	...
	initComponent: function() {
		 ...
		 if(!Ext.isEmpty(Ext.getCmp("addButton"))){
			 Ext.getCmp("addButton").on("click",function(){
				 ...
				 console.log(Ext.getCmp("billFormPanel").getForm().findField("writerDepartmentCode"));
				 win.show();
				 ...
				 var records = grid.getSelectionModel().getSelections();
	 			 ...
	 			 Ext.MessageBox.confirm('确认', '您确定要删除所选择的记录吗?', function(btn){
					if(btn == 'yes'){
			 			 ...
			 			 Ext.ux.Ajax.request("budget/projectBudgetDocument/delete.do", {ids: ids}, function(action, form){
		 ...
		 if(!Ext.isEmpty(Ext.getCmp("exportButton"))){
			 Ext.getCmp("exportButton").on("click", function(){
				 ...
				 window.open(basePath+"/budget/projectBudgetDocument/excelExport.do?id="+records[0].id);
			 });
		 }
		...
		buttons : [{
			...
			handler : function() {
				...
				if (btn == 'yes') {
					...
					//console.log(values);
					values.ids = ids;
					Ext.ux.Ajax.request("budget/projectBudgetDocument/submit.do",values,
