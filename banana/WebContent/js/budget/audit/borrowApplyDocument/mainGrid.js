mainGrid=Ext.extend(Ext.grid.GridPanel,{
    ...
    initComponent: function() {
		   ...
		   reader:new Ext.data.JsonReader({
			...
			pageSize: pageSize,
			 ...
			 proxy: new Ext.data.HttpProxy ({ 
				url:'budget/borrowApplyDocument/queryAudit.do'
			 })      
	 	});
		store.on({
		...
		var columns=[
	        ...
	        {header: "<center>借款金额</center>", width: 100, sortable: true, xtype: 'numbercolumn',
	        	dataIndex: 'totalMoney', format: '0.00', align:"right"},
	        {header: "<center>已还金额</center>", width: 100, sortable: true, xtype: 'numbercolumn',
	        	dataIndex: 'repaymentMoney', format: '0.00', align:"right"},
	        {header: "<center>剩余金额</center>", width: 100, sortable: true, 
	        	dataIndex: 'repaymentMoney', format: '0.00', align:"right",renderer:function(v, metaData, record){
	        		if(v=='null'){
		      			return '';
		      		}else{
		      			return Ext.util.Format.number((record.data.totalMoney-record.data.repaymentMoney), "0.00");
		      		}
	        	}
	        },
