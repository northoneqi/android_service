			            	...
			            	Ext.MessageBox.confirm('确认', '您确定同意审核所选择的记录吗?', function(btn){
								if(btn == 'yes'){
						 			 ...
						 			 for(var i = 0; i < records.length; i++){
				 			 		 ...
				 			 		 values.ids = ids;
						 			 Ext.ux.Ajax.request("budget/feeYearDocument/audit.do", values, function(action, form){
			            	...
			            	Ext.MessageBox.confirm('确认', '您确定驳回所选择的记录吗?', function(btn){
								if(btn == 'yes'){
						 			 ...
						 			 for(var i = 0; i < records.length; i++){
				 			 		 ...
				 			 		 var values = userForm.getForm().getValues();
				 			 		 values.ids = ids;
						 			 Ext.ux.Ajax.request("budget/feeYearDocument/comeBack.do", values, function(action, form){
										 var response =  Ext.util.JSON.decode(action.responseText);
										 if(Ext.isEmpty(response)){
				 ...
				 Ext.ux.Form.loadForm(Ext.getCmp("billFormPanel"), 
					 "budget/feeYearDocument/find.do", 
	 				 ...
	 				 function(form, action){
	 					 Ext.getCmp("workflowGrid").getStore().load({params: {id:records[0].id, billType: 2}});
			 ...
			 Ext.getCmp("stopButton").on("click",function(){
				 ...
				 Ext.MessageBox.confirm('确认', '您确定关闭所选择的记录吗?', function(btn){
					if(btn == 'yes'){
			 			 ...
			 			 Ext.ux.Ajax.request("budget/feeYearDocument/close.do", {ids:ids}, function(action, form){
							 var response =  Ext.util.JSON.decode(action.responseText);
							 if(Ext.isEmpty(response)){
			 ...
			 Ext.getCmp("enterButton").on("click",function(){
				 ...
				 Ext.MessageBox.confirm('确认', '您确定入账所选择的记录吗?', function(btn){
					if(btn == 'yes'){
			 			 ...
			 			 Ext.ux.Ajax.request("budget/feeYearDocument/enter.do", {ids:ids}, function(action, form){
							 var response =  Ext.util.JSON.decode(action.responseText);
							 if(Ext.isEmpty(response)){
