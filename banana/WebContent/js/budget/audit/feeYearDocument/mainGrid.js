mainGrid=Ext.extend(Ext.grid.GridPanel,{
    ...
    initComponent: function() {
		   ...
		   reader:new Ext.data.JsonReader({
			...
			pageSize: pageSize,
			 ...
			 proxy: new Ext.data.HttpProxy ({ 
				url:'budget/feeYearDocument/queryAudit.do'
			 })      
	 	});
		store.on({
		...
		var columns=[
	  	 	sm,
	  	 	new Ext.grid.RowNumberer({header:"<center>序号</center>",width:50,sortable : true,loccked:true,align:"center"}),
	        {header: "<center>单据编号</center>", width: 120, sortable: true, dataIndex: 'documentNum',loccked:true,renderer:function(v){
	      		if(v=='null'){
	      			return '';
	      		}else{
	      			return v;
