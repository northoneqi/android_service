manage = Ext.extend(manageUi,{
	initComponent: function() {
			            	...
			            	Ext.MessageBox.confirm('确认', '您确定同意审核所选择的记录吗?', function(btn){
								if(btn == 'yes'){
						 			 ...
						 			 for(var i = 0; i < records.length; i++){
				 			 		 ...
				 			 		 values.ids = ids;
						 			 Ext.ux.Ajax.request("budget/projectBudgetDocument/audit.do", values, function(action, form){
			            	...
			            	Ext.MessageBox.confirm('确认', '您确定驳回所选择的记录吗?', function(btn){
								if(btn == 'yes'){
						 			 ...
						 			 for(var i = 0; i < records.length; i++){
				 			 		 ...
				 			 		 var values = userForm.getForm().getValues();
				 			 		 values.ids = ids;
						 			 Ext.ux.Ajax.request("budget/projectBudgetDocument/comeBack.do", values, function(action, form){
										 var response =  Ext.util.JSON.decode(action.responseText);
										 if(Ext.isEmpty(response)){
		 ...
		 if(!Ext.isEmpty(Ext.getCmp("scanButton"))) {
			 Ext.getCmp("scanButton").on("click",function(){
				 ...
				 Ext.ux.Form.loadForm(Ext.getCmp("billFormPanel"), 
					 "budget/projectBudgetDocument/find.do", 
	 				 {id: records[0].id},
			 ...
			 Ext.getCmp("stopButton").on("click",function(){
				 ...
				 Ext.MessageBox.confirm('确认', '您确定关闭所选择的记录吗?', function(btn){
					if(btn == 'yes'){
			 			 ...
			 			 Ext.ux.Ajax.request("budget/projectBudgetDocument/close.do", {ids:ids}, function(action, form){
							 var response =  Ext.util.JSON.decode(action.responseText);
							 if(Ext.isEmpty(response)){
			 ...
			 Ext.getCmp("enterButton").on("click",function(){
				 ...
				 Ext.MessageBox.confirm('确认', '您确定入账所选择的记录吗?', function(btn){
					if(btn == 'yes'){
			 			 ...
			 			 Ext.ux.Ajax.request("budget/projectBudgetDocument/enter.do", {ids:ids}, function(action, form){
							 var response =  Ext.util.JSON.decode(action.responseText);
							 if(Ext.isEmpty(response)){
		 ...
		 //反入账
		 if(!Ext.isEmpty(Ext.getCmp("disEnterButton"))) {
			 Ext.getCmp("disEnterButton").on("click",function(){
				 var records = grid.getSelectionModel().getSelections();
	 			 if(records.length == 0){
	 				 Ext.ux.Msg.warning("请至少选择一条记录！"); 
	 				 return;
	 			 ...
	 			 for(var i = 0; i < records.length; i++){
	 				 var data = records[i].data;
	 				 if(!data.entered) {
	 					 Ext.ux.Msg.warning("您所选择的记录包含未入账的单据，无法反入账！");
	 				 	 return;
	 				 }
	 			 }
	 			 
				 Ext.MessageBox.confirm('确认', '您确定反入账所选择的记录吗?', function(btn){
					if(btn == 'yes'){
			 			 var ids = [];
			 			 for(var i = 0; i < records.length; i++){
			 				 ids.push(records[i].id);
			 			 ...
			 			 Ext.ux.Ajax.request("budget/projectBudgetDocument/disEnter.do", {ids:ids}, function(action, form){
							 var response =  Ext.util.JSON.decode(action.responseText);
							 if(Ext.isEmpty(response)){
								 Ext.ux.Msg.info("反入账成功");
							 }else{
								 Ext.ux.Msg.info(response.data.instruction);
							 }
							 grid.getStore().reload();
		 ...
		 if(!Ext.isEmpty(Ext.getCmp("printButton"))){
			 Ext.getCmp("printButton").on("click", function(){
				 var records = grid.getSelectionModel().getSelections();
	 			 ...
	 			 printReport(records[0].id, "project_budget_document");
			 });
		 }
