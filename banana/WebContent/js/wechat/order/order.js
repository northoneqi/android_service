...
order=Ext.extend(orderUi,{
	initComponent: function() {
		...
		Ext.getCmp("add_button").on("click",function(){
			 ...
			 win.title = "职级变动登记表<span style='color:red'>[增加]</span>";
			 win.show();
		...
		Ext.getCmp("edit_button").on("click",function(){
			 ...
			 var c = rowid.split(",");
			 if(rowid!="" && c.length == 1){
				 win.title = "职级变动登记表<span style='color:red'>[修改]</span>";
				 win.show();
				 Ext.getCmp("id").setValue(rowid);
				 ...
				 saveButtion.on("click",function(){
					...
					if(formPanel.isValid()){
						formPanel.submit({
							...
							waitTitle : '系统提示',// 标题
							method:'POST',
							url:ctx+"/testtt/zhaoxh/update.do",
							params:formPanel.getValue,
							success:function(form,action){
								grid.getStore().load();
		...
		Ext.getCmp("delete_button").on("click",function(){
			 ...
			 if(rowid!=""){
				 Ext.Msg.confirm('系统提示', '您确定要删除吗?', function (opt) {
	                 if (opt == 'yes') {
	                    Ext.Ajax.request({
							 url:ctx+"/testtt/zhaoxh/delete.do",
							 params:{ids:rowid},
