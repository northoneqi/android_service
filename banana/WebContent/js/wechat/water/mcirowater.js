mcirowater=Ext.extend(mcirowaterUi,{
	initComponent: function() {
		...
		Ext.getCmp("exportexcelButton").on("click",function(){
			window.location.href=ctx+'/wechat/mcirowater/exportCompExcel.do';
			Ext.Ajax.request({
		...
		Ext.getCmp("queryButton").on("click",function(){
			var searchForm = Ext.getCmp("searchForm").getForm();
			 var companyName=searchForm.findField("companyName").getValue();
			 ...
			 Ext.getCmp("mcirowaterGrid").getStore().baseParams={start: 0, limit: pageSize,companyName:companyName,startTime:startTime,endTime:endTime,state:state};
			 Ext.getCmp("mcirowaterGrid").getStore().load();
...
function fun_sp(id,type,msg){
	Ext.Ajax.request({
		url:ctx+"/wechat/mcirowater/updateCom.do",
