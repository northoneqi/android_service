...
Ext.QuickTips.init();
commodityRecord=Ext.extend(commodityRecordUi,{
	initComponent: function() {
		commodityRecord.superclass.initComponent.call(this);
		...
		//excel导出
		Ext.getCmp("Excel_button").on("click", function(){
			var formValues = Ext.getCmp("queryForm").getForm().getValues();		
			window.open(basePath+"/supplyChain/commodityRecord/goodsExcelStatics.do?"+jsonToParams(formValues));
		});
		/** 增加按钮点击事件 **/
				 ...
				 saveButtion.on("click",function(){
					...
					if(formPanel.isValid()){
						formPanel.submit({
							...
							waitTitle : '系统提示',// 标题
							method:'POST',
							url:ctx+"/supplyChain/commodityRecord/save.do",
		...
		Ext.getCmp("edit_button").on("click",function(){
			 ...
			 if(rowid!="" && c.length == 1){
				 win.show();
				 Ext.getCmp("commodityRecordId").setValue(rowid);
		...
		Ext.getCmp("delete_button").on("click",function(){
			 ...
			 if(rowid!=""){
				 Ext.Msg.confirm('系统提示', '您确定要删除吗?', function (opt) {
	                 if (opt == 'yes') {
	                    Ext.Ajax.request({
							 url:ctx+"/supplyChain/commodityRecord/delete.do",
							 params:{ids:rowid},
...
function loadForm(id) {
	...
	formPanel.load( {
		...
		waitTitle : '提示',//标题
		url:ctx+"/supplyChain/commodityRecord/load.do",
...
function getCheckRowId(){
	 ...
	 if(sm.getSelected()){
		 ...
		 for(var i = 0 ; i < recs.length ; i++){
			 rowId += recs[i].get("commodityRecordId");
